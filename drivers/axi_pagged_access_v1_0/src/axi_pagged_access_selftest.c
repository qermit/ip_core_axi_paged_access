...
XStatus AXI_PAGGED_ACCESS_Reg_SelfTest(void * baseaddr_p)
{
	...
	for (write_loop_index = 0 ; write_loop_index < 4; write_loop_index++)
	  AXI_PAGGED_ACCESS_mWriteReg (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
	for (read_loop_index = 0 ; read_loop_index < 4; read_loop_index++)
	  if ( AXI_PAGGED_ACCESS_mReadReg (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
...
XStatus AXI_PAGGED_ACCESS_Mem_SelfTest(void * baseaddr_p)
{
	...
	{
	  AXI_PAGGED_ACCESS_mWriteMemory(baseaddr+4*Index, (0xDEADBEEF % Index));
	}

	for ( Index = 0; Index < 16; Index++ )
	{
	  Mem32Value = AXI_PAGGED_ACCESS_mReadMemory(baseaddr+4*Index);
